  ...
  reducers: {
    pcbuilder: (state, action) => {
      ...
      if (!state.categories[category].some((item) => item.id === product.id)) {
        const isUniqueProduct = Object.values(state.categories).every(
          (categoryProducts) =>
            categoryProducts.every((item) => item.id !== product.id)
        )

        if (isUniqueProduct) {
          state.categories[category].push(product)
          const allCategoriesSelected = Object.values(state.categories).every(
